...
class ConfigurationTests(unittest.TestCase):
    ...
    @patch.dict(
        os.environ,
        {
            key: value
            for key, value
            in VALID_ENVIRONMENT.items()
            if key != "TWITCH_CLIENT_SECRET"
        },
        clear=True,
    )
    def test_missing_twitch_secret_is_rejected(
        self,
    ) -> None:
        settings = load_settings(env_file=None)
    ...
    def test_invalid_developer_id_is_rejected(
        self,
    ) -> None:
        with self.assertRaisesRegex(
            ConfigurationError,
    ...
    def test_missing_discord_guild_is_rejected(
        self,
    ) -> None:
        settings = load_settings(env_file=None)
